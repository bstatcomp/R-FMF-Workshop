...
All of the above can be simplified using the **dplyr** packages presented in the next chapter.
...
We used `geom_col` to generate the bars, see `?geom_col` for additionaly information. The `fill` aesthetic is more appropriate here than color, since color would only color the edges, while fill fills the whole area. Additionally, we used `position = "dodge"`, otherwise the bars would be stacked.
...
At first glance the difference is huge, while we already know that it is rather small. But someone without attention to detail might not notice this, and would believe our story. The only this we did is change the scale, similar to the example with stocks data. Additionally, this is the first time we used the `labs` layer, which allows us to rename the labels.
...
### iris data set
Let us start with a fairly basic data set.
```{r, warning = FALSE}
data(iris)
head(iris)
